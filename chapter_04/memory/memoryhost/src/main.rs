...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut f = 
     File::open("../memoryguest/target/wasm32-unknown-unknown/debug/memoryguest.wasm")?;
    let mut bytes = vec![];
...
fn get_vec_from_memory(rt: &Runtime, ptr: i32, len: i32) -> Vec<u8> {
    let data = unsafe { &*rt.memory() };
...
//     // tell the guest we set the name
// let set_name = module.find_function::<(i32, i32), i32>("set_name")?;
// let greeting_idx = 1; // 0 for "Hello" and 1 for "Ahoy"
// let new_len = set_name.call((greeting_idx, name.len() as i32))?;
// println!("New string length: {}", new_len);
